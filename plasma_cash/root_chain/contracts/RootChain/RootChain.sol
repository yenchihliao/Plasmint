...
contract RootChain {
    ...
    function checkAddr(address addr) returns (bool) {
        for (uint i = 0; i < authorityAddr.length; i++) {
            if (addr == authorityAddr[i]) {
                return true;
            }
        }
        return false;
    ...
    //    isAuthority
    {
        bytes32 msghash = keccak256(blkRoot, blknum, isDepositBlock, depositTx, depositTxProof);
        uint count = 0;
        for (uint i = 0; i < v.length; i++) {
            address addr = ecrecover(msghash, v[i], r[i], s[i]);
            if (checkAddr(addr)) {
                count += 1;
            }
        }
