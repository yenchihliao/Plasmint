...
@api.route('/check_tx', methods=['POST'])
def check_tx():
    tx = request.form['tx']
    return container.get_child_chain().check_transaction(tx)
...
@operator.route('/commit', methods=['POST'])
def commit():
    return container.get_child_chain().commit()
